...
public:
	...
	bool insertionDroitePlan(Droite& droite, Vecteur3d& pointCollision) {
		if (normale.produitScalaire(droite.obtenirVecteurDirecteur()) != 0) {
			...
			pointCollision = Vecteur3d((droite.obtenirPoint().x + (k * droite.obtenirVecteurDirecteur().x)), (droite.obtenirPoint().y + (k * droite.obtenirVecteurDirecteur().y)), (droite.obtenirPoint().z + (k * droite.obtenirVecteurDirecteur().z)));
			return true;
		}
		return false;
	}
