...
public:
	...
	void creer() {
		...
		for (int i = 0; i < cheminsObjet.size(); ++i){
			...
			if (obj.type == ITEM){
				if (obj.IDitem == 0 || obj.IDitem == 1 || obj.IDitem == 2 || obj.IDitem == 3 || obj.IDitem == 10 || obj.IDitem == 11 || obj.IDitem == 12 || obj.IDitem == 80 || obj.IDitem == 72){
	...
	Vecteur3d debut(double& hAngle, double& vAngle) {
		...
		InfoObjet cle;
		LecteurFichier::lireObjet("Ressources/Info/ClefRoulliee.txt", cle);
		cle.direction = { 0, 0, 0 };
		cle.ID = 9;
		cle.largeur = 0;
		cle.position = { -1.5, 0.1, -1 };
		cle.rotation = { 0, -90, 0 };
		cle.type = ITEM;
		cle.IDitem = 70;
		
		salleDebut.Objet.push_back(new InfoObjet(cle));
		
			// Companion
		
		InfoObjet companion;
		LecteurFichier::lireObjet("Ressources/Info/thai.txt", companion); //??? wtf.
		companion.direction = { 0, 0, 0 };
		companion.ID = 8;
		companion.largeur = 0;
		companion.position = { -3.9, 1.3, -3.0 };
		companion.IDitem = 42;
